...
const HookQ2 = () => {
    ...
    */
    ...
    const todoTag = useRef(null); //todo태그
    ...
    const todoRegister = () => { // 클릭이벤트
        ...
        setTodoData({ ...todoData, todo: '', list: newArr }); // 원래 todoData를 복사해오고(복사해올 필요는 없다), todo를 공백으로 바꾼다. 이미 list에 담았기 때문이다. 그리고 list에는 반환받은 newArr를 담는다.
        // IterationComponent2와 동일한 원리
    ...
    let newList = todoData.list.map((item, index) => <li key={index}>{index+1}번-{item}</li>); // 배열에다가 li태그 붙여서 반환하고, 이 데이터를 화면에 뿌릴 것

    return (
        <>
            <hr />
            <h3>ref로 할일목록 만들기</h3>
            <input type="text" name="todo" placeholder="할일목록" ref={todoTag} onChange={handleTodoInput} value={todoData.todo} onKeyDown={pressEnter}/>
